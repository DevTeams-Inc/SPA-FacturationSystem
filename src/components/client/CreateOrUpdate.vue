...
export default {
  ...
  methods: {
    get(id) {
      ...
      self.loading = true;
      self.$store.state.services.ClientService.get(id)
        .then(r => {
          self.loading = false;
      ...
      self.$refs[form].validate(valid => {
        if (valid) {
          ...
          if (self.form.clientId > 0) {
            self.$store.state.services.ClientService.update(self.form)
              .then(r => {
                self.loading = false;
            ...
            self.$store.state.services.ClientService.add(self.form)
              .then(r => {
                self.loading = false;
                self.$Notice.success({
